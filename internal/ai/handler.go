...
func (h *Handler) ConfigureTenant(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		sharedhandler.WriteValidationError(c, "invalid AI tenant settings payload", err)
		return
	}
...
func (h *Handler) ConfigureInstance(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&input); err != nil {
		sharedhandler.WriteValidationError(c, "invalid AI instance settings payload", err)
		return
	}
