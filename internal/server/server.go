...
func New(cfg *config.Config, app *service.Application, logger *slog.Logger, db *sql.DB) *Server {
	...
	protected.Use(middleware.Audit(app.Audit))
	{
		...
		// Fase B: rich message operations (n8n Evolution API node compat)
		protected.POST("/message/sendReaction/:instanceName", middleware.RequireRoles(opRoles...), messageLimiter.Middleware(), instanceHandler.LegacySendReaction)
		protected.POST("/message/sendPoll/:instanceName", middleware.RequireRoles(opRoles...), messageLimiter.Middleware(), instanceHandler.LegacySendPoll)
		protected.POST("/message/sendContact/:instanceName", middleware.RequireRoles(opRoles...), messageLimiter.Middleware(), instanceHandler.LegacySendContact)
		protected.POST("/chat/whatsappNumbers/:instanceName", middleware.RequireRoles(readRoles...), instanceHandler.LegacyCheckNumbers)
		protected.POST("/chat/markMessageAsRead/:instanceName", middleware.RequireRoles(opRoles...), instanceHandler.LegacyMarkRead)
