...
func New(cfg *config.Config, app *service.Application, logger *slog.Logger, db *sql.DB) *Server {
	...
	protected.Use(tenantMiddleware.Resolve())
	{
		...
		protected.POST("/instance/setPresence/:instanceName", middleware.RequireRoles(auth.RoleOwner, auth.RoleAdmin, auth.RoleAgent), instanceHandler.LegacySetPresence)
		...
		protected.POST("/message/presence/:instanceName", middleware.RequireRoles(auth.RoleOwner, auth.RoleAdmin, auth.RoleAgent), instanceHandler.LegacyChatPresence)
		protected.POST("/message/markread/:instanceName", middleware.RequireRoles(auth.RoleOwner, auth.RoleAdmin, auth.RoleAgent), instanceHandler.LegacyMarkRead)
